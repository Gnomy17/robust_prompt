...
from parser_cifar100 import get_args
...
def train_adv(args, model, ds_train, ds_test, logger):
    ...
    if mixup_active :
        mixup_fn = Mixup(
            ...
            label_smoothing=args.labelsmoothvalue, num_classes=nclasses)
...
# pgd_loss, pgd_acc = evaluate_pgd(args, model, test_loader, prompt=None, a_lam=args.a_lam)
# logger.info('PGD10 (Naive) : loss {:.4f} acc {:.4f}'.format(pgd_loss, pgd_acc))
...
logger.info('PGD10 (Adaptive) : loss {:.4f} acc {:.4f}'.format(pgd_loss, pgd_acc))
